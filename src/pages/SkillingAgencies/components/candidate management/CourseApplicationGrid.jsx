...
const CourseApplicationGrid = ({ toolbarStatus = true, handleSwitchPane , SelectedOp }) => {
    ...
    useEffect(() => {
        setData(courseAppList);

        console.log("Application Data",courseAppList)
    }, [courseAppList]);
...
//whenever response api is called the questions of thje jd is acquired and stored in a array 
        let questionHeader = [];
        const questions = Array.isArray(SelectedOp.selectedCourse?.Questions)
  ? SelectedOp.selectedCourse.Questions
  : [];
        ...
        if (questions?.length > 0) {
            questions.forEach((qn) => {
                ...
                if (qn.qnLabel && qn.cid) {
                    qnOb["key"] = qn.cid;
                    qnOb["label"] = qn.cLabel;
                ...
                // }));
                    let apiBody = {
                                ...
                                payload: {                                   
                                    ...
                                    status: currentStatus && currentStatus.length > 0 && currentStatus.trim() !== "*" ? currentStatus : null,                                  
    ...
    }, [searchQuery,questionFilter,currentStatus]);
...
//console.log("Select Course Values",SelectedOp?.selectedCourse?.preRequisiteSkillsList?.[SelectedOp?.selectedCourse?.preRequisiteSkillsList?.length].totalCount);
  useEffect(() => {   
         setRowSelection({});
        if (currentStatus !== "*") {
    ...
    const table = useMaterialReactTable({
        ...
        renderRowActionMenuItems: ({ closeMenu, row }) => [
            <OpportunitiesGridActions row={row} setRefresh={setRefresh} allowShortlist={true} allowReject={true} allowScreening={true} allowSelect={true} 
                 closeMenu={() => closeMenu()} // this function handles closing of menu item popup
            />,
...
export default React.memo(CourseApplicationGrid);
