...
const AchivementsHistory = () => {
  ...
  const checkDuplicate = () => {
    ...
    achievementHistory?.data?.map((cert) => {
      if (
        cert.title === skillingDetails.awardName &&
  ...
  const handleSkillingAdd = (close) => {
      ...
      showErrorToast(
        (
          content[selectedLanguage]?.find(
            (item) => item.elementLabel === "AwardAlreadyExistDate"
          ) || {}
        ).mvalue || "nf Award already exist with this date"
      );
      return;
    }
    ...
    PostApi("Awards", {
      ...skillingDetails,
      location: location,
      date: FormatDateIntoPost(skillingDetails.date),
      mlanguage: selectedLanguage,
      awardName: skillingDetails.awardName,
    })
      .then((res) => {
        dispatch(
          addNewAwards({
            ...res.data,
            date: convertDateToMilliseconds(res.data.date),
          })
        );
        ...
        showSuccessToast(
          (
            content[selectedLanguage]?.find(
              (item) => item.elementLabel === "NewAwardsAddedSuccessful"
            ) || {}
          ).mvalue || "nf New Awards Added Successful"
        );
                    ...
                    <div className=" ">
                      <div class="mb-2 ">
                        ...
                        <input
                          ...
                          id=""
                          placeholder=""
                          value={skillingDetails.awardName}
                          onChange={(e) =>
                            setSkillingDetails({
                              ...skillingDetails,
