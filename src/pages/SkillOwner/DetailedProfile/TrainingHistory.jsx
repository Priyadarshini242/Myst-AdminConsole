...
import TrainingSummary from "../../../components/SkillOwner/DetailedProfile/TrainingSummary";
import { useSelector } from "react-redux";
import TrainingDetail from "../../../components/SkillOwner/DetailedProfile/TrainingDetail";
...
const TrainingHistory = () => {
  ...
  const [isCustomInstitutionTraining, setIsCustomInstitutionTraining] =
    useState(false);

  /* STORE IMPORTS */
  const { regionalData } = useSelector((state) => state);

  const handleInsTrainingParentToChild = (val) => {
    setIsCustomInstitutionTraining(val);
  };
  ...
  const checkDuplicate = () => {
    ...
    trainingHistory?.data?.map((cert) => {
      if (
        cert.title === skillingDetails?.trainingName &&
  ...
  const handleSkillingAdd = (close) => {
    ...
    console.log("ksh shh ishih ihih", location);
    /* SHOW ERR MSG FOR REQ FIELDS */
    if (
      !location ||
      !skillingDetails?.organization ||
    ...
    if (duplicate) {
      showErrorToast("Training already exist in within the date range");
      return;
    }
    ...
    PostApi("Training", {
      ...
      .then((res) => {
        ...
        const data = res?.data;
        if (isCustomInstitutionTraining) {
        ...
        setIsCustomInstitutionTraining(false);
      })
      .catch((err) => {
        console.log(err);
        showErrorToast(
                    ...
                    <div className=" ">
                      <div class="mb-2 ">
                        ...
                        >
                          {(
                            content[selectedLanguage]?.find(
                              (item) => item.elementLabel === "NameOfTheSkill"
                            ) || {}
                          ).mvalue || "Name Of Training"}
                          <span className="text-danger"> *</span>
                        </label>
                        <input
                          type="text"
                          ...
                          id=""
                          placeholder=""
                          value={skillingDetails?.trainingName}
                          onChange={(e) =>
                            setSkillingDetails({
                              ...skillingDetails,
                        ...
                        >
                          <div className="w-75 ">
                            {(
                              content[selectedLanguage]?.find(
                                (item) =>
                                  item.elementLabel === "TrainingHistory"
                              ) || {}
                            ).mvalue || "nf Training History"}
                          </div>
