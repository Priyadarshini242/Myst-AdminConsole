...
import LicenseSummary from "../../../components/SkillOwner/DetailedProfile/LicenseSummary";
import LicenseDetail from "../../../components/SkillOwner/DetailedProfile/LicenseDetail";
import { fetchLicensesHistory } from "../../../api/fetchAllData/fetchLicensesHistory";
import { addNewLicenses } from "../../../reducer/detailedProfile/licensesSlice";
...
const LicenseHistory = () => {
  ...
  console.log("licensesHistory", licensesHistory);
  ...
  const [isCustomInstitutionLicense, setIsCustomInstitutionLicense] =
    useState(false);

  const handleOrgLicenseParentToChild = (val) => {
    setIsCustomInstitutionLicense(val);
  };
  ...
  const checkDuplicate = () => {
    ...
    licensesHistory?.data?.map((cert) => {
      if (
        cert.title === skillingDetails.title &&
  ...
  const handleSkillingAdd = (close) => {
      ...
      showErrorToast(
        (
          content[selectedLanguage]?.find(
            (item) => item.elementLabel === "LicenseAlreadyExistDate"
          ) || {}
        ).mvalue || "nf License already exist in within the date range"
      );
      return;
    }
    ...
    PostApi("Licenses", {
      ...
      endDate: FormatDateIntoPost(skillingDetails.toDate),
      validFrom: FormatDateIntoPost(skillingDetails.fromDate),
      validTill: FormatDateIntoPost(skillingDetails.toDate),
      ...
      .then((res) => {
        dispatch(
          addNewLicenses({
            ...res.data,
            startDate: convertDateToMilliseconds(res.data.validFrom),
            endDate: convertDateToMilliseconds(res.data.validTill),
          })
        );

        /* IF NEW ORGANIZATION NAME ENTERED */
        const data = res?.data;
        if (isCustomInstitutionLicense) {
        ...
        showSuccessToast(
          (
            content[selectedLanguage]?.find(
              (item) => item.elementLabel === "NewLicenseAddedSuccessful"
            ) || {}
          ).mvalue || "nf New license Added Successful"
        );
        ...
        /* RESET THE STATE OF THE NEWLY ADDED ORG NAME IF INSERTED */
        setIsCustomInstitutionLicense(false);
      })
      .catch((err) => {
        console.log(err);
        showErrorToast(
            ...
            <div className="modal-content">
                    ...
                    <div className=" ">
                      <div class="mb-2 ">
                        ...
                        >
                          {(
                            content[selectedLanguage]?.find(
                              (item) => item.elementLabel === "LicenseName"
                            ) || {}
                          ).mvalue || "nf License Name"}
                          <span className="text-danger"> *</span>
                        </label>
                        <input
                          type="text"
                          ...
                          id=""
                          placeholder=""
                          value={skillingDetails.licenseName}
                          onChange={(e) =>
                            setSkillingDetails({
                              ...skillingDetails,
                      ...
                      <div className="d-flex my-2  w-100   ">
                        ...
                        <div className=" ms-2 h-75 w-100  ">
                          <div>
                            ...
                            {/* <input type="date" style={{ height: "32px" }} value={skillingDetails.toDate} name='toDate' className="form-control bg-body-tertiary h-75 " min={skillingDetails.fromDate} max={timestampToYYYYMMDD(Date.now())} id="exampleFormControlInput1" onChange={(e) => setSkillingDetails({ ...skillingDetails, [e.target.name]: e.target.value })} /> */}
                            <DatePicker
                              id="exampleFormControlInput1"
                              style={{ height: "32px" }}
                              className={`form-control  h-75 buttom-line-input px-0 w-100 `}
                          ...
                          {/* <div className={onGoing.instituteEndDate ? 'd-flex ms-1 align-items-center  font-6 text-secondary   ' : 'd-flex ms-1 align-items-center font-6 text-secondary '} >
                                                    <label htmlFor="exampleFormControlInput1" className="">Current Skilling </label>
                                                    <input className='ms-2' type="checkbox" name="instituteEndDate"
                                                        onChange={(e) => {
                                                            setOnGoing({ ...onGoing, [e.target.name]: e.target.checked });
                                                            if (e.target.checked) {
                                                                setSkillingDetails({ ...skillingDetails, endDate: timestampToYYYYMMDD(Date.now()) });
                                                            }
                                                        }} />
                                                </div> */}
                        </div>
              ...
              <div className="modal-footer d-flex justify-content-end  ">
                <div className="d-flex gap-2">
                  ...
                  {/* <button type="button" className="btn  font-5" style={{ color: "var(--primary-color)", backgroundColor: "#EFF5DC" }} onClick={() => handleSkillingAdd(false)}>save</button>
                                    <button type="button" className="btn  me-2 font-5" style={{ backgroundColor: "var(--primary-color)", color: "#EFF5DC" }} data-bs-dismiss="modal" onClick={() => handleSkillingAdd(false)}>Save & Close</button> */}
                </div>
              </div>
            </div>
          </div>
                      ...
                      <h2 className="accordion-header py-1 ">
                        ...
                        >
                          <div className="w-75 ">
                            ...
                            {(
                              content[selectedLanguage]?.find(
                                (item) => item.elementLabel === "Licenses"
                              ) || {}
                            ).mvalue || "nf Licenses"}
                          </div>
