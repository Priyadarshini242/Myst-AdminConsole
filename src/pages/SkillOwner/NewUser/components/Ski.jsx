...
const Ski = () => {
  ...
  const handleSkillingAdd = (close) => {
    ...
    if (!skillingDetails.title) {
      newErrors.title = "Skilling name is required";
    }
    ...
    <>
      <div class="mb-2 " style={{ flex: "1 1 15rem" }}>
        ...
        <input
          ...
          id=""
          placeholder="Enter skilling name"
      ...
      <div class="mb-2 " style={{ flex: "1 1 15rem" }}>
        ...
        <CreateSelectInstitution
          ...
          showDropdown={false}
          formType={"Skilling"}
          handleInsTSkillingParentToChild={handleInsTSkillingParentToChild}
      ...
      {/*   <div className='d-flex flex-sm-row flex-column justify-content-center gap-3 align-items-start   ' style={{ flex: '1 1 25rem' }} >
                <div className=" h-75 w-100" >
                    ...
                    <input type="date" style={{ height: "32px" }} value={skillingDetails.fromDate} max={timestampToYYYYMMDD(Date.now())} name='fromDate' className="buttom-line-input form-control  h-75 px-0 " id="exampleFormControlInput1" onChange={(e) => setSkillingDetails({ ...skillingDetails, [e.target.name]: e.target.value })} />
                </div>
                <div className=" h-75 w-100  ">
                    ...
                    <div className={onGoing.instituteEndDate ? 'd-flex ms-1 align-items-center  font-6 text-secondary   ' : 'd-flex ms-1 align-items-center font-6 text-secondary '} >
                        <label htmlFor="exampleFormControlInput1" className="">{(content[selectedLanguage]?.find(item => item.elementLabel === 'CurrentSkilling') || {}).mvalue || "nf CurrentSkilling"}</label>
        ...
        <div className=" h-75 w-100  ">
          ...
          >
            <label htmlFor="exampleFormControlInput1" className="">
              {(
                content[selectedLanguage]?.find(
                  (item) => item.elementLabel === "CurrentSkilling"
                ) || {}
              ).mvalue || "nf CurrentSkilling"}
            </label>
            <input
              className="ms-2"
              type="checkbox"
