...
const ResumeSkilling = ({ skilling, setResumeSkilling, index }) => {
  ...
  useEffect(() => {
    const currentAccordion = accordionStates[skilling.id];

    if (currentAccordion?.institutionFirstWord?.trim()?.length > 2) {
      const controller = new AbortController();
      const signal = controller.signal;

      updateAccordionState(skilling.id, "isApiLoader", true);
      ...
      debouncedApiRequest(
        ...
        (data) => {
          ...
          updateAccordionState(skilling.id, "educationSuggestionsData", formattedData || []);
          updateAccordionState(skilling.id, "isApiLoader", false);
        },
        () => updateAccordionState(skilling.id, "isApiLoader", false),
        signal
      );
      ...
      updateAccordionState(skilling.id, "educationSuggestionsData", []);
    }
  }, [accordionStates[skilling.id]?.institutionFirstWord, skilling.id, selectedLanguage, skilling?.edit]);

  const handleEducationChange = (selectedOption) => {
    const isExisting = accordionStates[skilling.id]?.educationApiData?.some(
  ...
  return (
    <div className="d-md-flex d-block gap-2 align-items-start">
      <input type="checkbox" className='' style={{ marginTop: '1.3rem' }} name="" id="" checked={skilling?.active || skilling?.import}
        disabled={skilling?.import}
        onChange={(e) => {
          setResumeSkilling((prevSkillings) => {
            return prevSkillings.map((item) =>
              item.id === skilling.id ? { ...item, active: e.target.checked } : item
            );
          });
        }}
      />
        ...
        <div class="accordion-item  ">
          <h2 class="accordion-header ">
            <button
              ...
              data-bs-target={`#panelsStayOpen-collapseEdu${skilling?.id}`}
              aria-expanded="false"
              aria-controls={`panelsStayOpen-collapseEdu${skilling?.id}`}
              onClick={() => {
                if (!accordionStates[skilling.id]?.isOpen) {
                  updateAccordionState(skilling.id, "isOpen", true);
                  updateAccordionState(skilling.id, "institutionFirstWord", skilling?.institution || "");
                  updateAccordionState(skilling.id, "institution", skilling?.institution || "");
                }
              }}
            >
              {contentLabel('Skilling', 'nf Skilling')}-{index + 1}
            </button>
          </h2>
          <div
            id={`panelsStayOpen-collapseEdu${skilling?.id}`}
            class={`accordion-collapse collapse `}
          >
            <div class="accordion-body" style={{
              position: "relative",
              opacity: skilling?.import ? '.5' : '1',
              pointerEvents: skilling?.import ? 'none' : ''
            }}>

              <Row
                className=""
                style={{ pointerEvents: skilling.edit ? "" : "none" }}
              >
                ...
                <Col  md={6} xs={12}  className=" mb-2">
                  <label htmlFor="" className="form-label"> {contentLabel('SkillingName', 'nf Skilling Name')}<span className="text-danger ms-1">*</span></label>
                  <input
                    type="Text"
                    class="form-control"
                    id="cert-name"
                    placeholder="Enter your training name"
                    value={skilling?.skillingName}
                    onChange={(e) => {
                      setResumeSkilling((prevSkillings) => {
                        return prevSkillings.map((item) =>
                          item.id === skilling.id
                            ? { ...item, skillingName: e.target.value }
                            : item
                        );
                      });
                    }}
                  />
                        <InputLimit value={skilling?.skillingName}/>
                </Col>
                ...
                <Col md={6} xs={12}  className="mb-4">
                  ...
                  <CreatableSelect
                    ...
                    onInputChange={(value) => {
                      ...
                      updateAccordionState(skilling.id, "institutionFirstWord", value);
                    }}
                    onChange={handleEducationChange}
                    // options={accordionStates[skilling.id]?.educationSuggestionsData || []}
                    options={(() => {
                      const options = accordionStates[skilling.id]?.educationSuggestionsData || [];
                      const trimmedInputValue = accordionStates[skilling.id]?.institutionFirstWord?.trim().toLowerCase() || "";
                    ...
                    isLoading={accordionStates[skilling.id]?.isApiLoader}
                      menuIsOpen={accordionStates[skilling.id]?.isApiLoader ? false : undefined}
                    inputValue={accordionStates[skilling.id]?.institutionFirstWord || ''}
                    value={
                      accordionStates[skilling.id]?.institution
                        ? {
                          label: accordionStates[skilling.id]?.institution,
                          value: accordionStates[skilling.id]?.institution,
                        }
                        : null
                    }
                    // menuIsOpen={true}
    ...
    ?.join(" ")
          ...
          <span style={{ color: "var(--primary-color)" }}>{contentLabel('Create', 'nf Create')}</span>
                    ...
                    <div className="mb-3 d-flex flex-column justify-content-between">
                      <label className="text-label form-label d-flex align-items-center white-space-no-wrap">
                        {(
                          ...
                          "nf When did you start your Skilling "}{" "}
                        <span className="text-danger ms-1">*</span>
                      </label>
                      ...
                      <div className=" ">
                        <DatePicker
                          ...
                          selected={skilling?.startYear ? new Date(parseInt(new Date(skilling?.startYear, 0, 1).getTime())) : null}
                          onChange={(date) => {
                  ...
                  <Col xs={6} className="ps-1 ps-md-3">
                    <div className="mb-4 d-flex flex-column justify-content-between">
                      ...
                      isParser={true}
                        ...
                        selectedMonthNumber={skilling.startMonth}
                        selectedYear={new Date(skilling?.startYear, 0, 1)}
                      />
                    </div>
                  </Col>
                </Col>
                  ...
                  <div className="mb-4">
                    <label className="d-flex justify-content-between form-label text-label">
                      ...
                      <div className="d-flex align-items-center justify-content-center">
                        <input
                          ...
                          type="checkbox"
                          name="online"
                          checked={skilling?.location?.includes("Online")}
                          onChange={(e) => {
                            setOnline(!online);
                          }}
                        />
                    ...
                    <MultiSelect
                      setLocationData={(newLocation) => {
                        setResumeSkilling((prevSkilling) => {
                          return prevSkilling.map((item) =>
                            item.id === skilling.id ? { ...item, location: newLocation } : item
                          );
                        });
                      }}
                      viewLocation={skilling?.location}
                      onlineStatus={online}
                    />
                ...
                <Col class="mb-3">
                  <BriefDescriptionTextArea
                    label={contentLabel(
                      "BriefDescriptionSkilling",
                      "nf Brief decription about skilling"
                    )}
                    htmlFor={"SkillingBriefDescription"}
                    isRequired={false}
                    textareaClass={""}
                    id={"SkillingBriefDescription"}
                    name={"briefDescription"}
                    value={skilling?.description}
                    onChange={(e) => {
                      setResumeSkilling((prevSkillings) => {
                        return prevSkillings.map((item) =>
                          item.id === skilling.id
