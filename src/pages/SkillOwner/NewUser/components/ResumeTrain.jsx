...
const ResumeTrain = ({ train, setResumeTrain, index }) => {
  ...
  useEffect(() => {
    const currentAccordion = accordionStates[train.id];

    if (currentAccordion?.institutionFirstWord?.trim()?.length > 2) {
      const controller = new AbortController();
      const signal = controller.signal;

      updateAccordionState(train.id, "isApiLoader", true);
      ...
      debouncedApiRequest(
        ...
        (data) => {
          ...
          updateAccordionState(train.id, "isApiLoader", false);
        },
        () => updateAccordionState(train.id, "isApiLoader", false),
        signal
      );
      ...
      updateAccordionState(train.id, "educationSuggestionsData", []);
    }
  }, [
    accordionStates[train.id]?.institutionFirstWord,
    train.id,
    selectedLanguage,
    train?.edit,
  ]);
  ...
  const handleEducationChange = (selectedOption) => {
    const isExisting = accordionStates[train.id]?.educationApiData?.some(
  ...
  return (
    <div className="d-md-flex d-block gap-2 align-items-start">
      <input
        ...
        name=""
        id=""
        checked={train?.active || train?.import}
        disabled={train?.import}
        onChange={(e) => {
          setResumeTrain((prevTrainings) => {
            return prevTrainings.map((item) =>
              item.id === train.id
        ...
        <div class="accordion-item  ">
          <h2 class="accordion-header ">
            <button
              ...
              data-bs-target={`#panelsStayOpen-collapseEdu${train?.id}`}
              aria-expanded="false"
              aria-controls={`panelsStayOpen-collapseEdu${train?.id}`}
              onClick={() => {
                if (!accordionStates[train.id]?.isOpen) {
                  updateAccordionState(train.id, "isOpen", true);
                  updateAccordionState(
                    train.id,
            ...
            >
              {contentLabel("Training", "nf Training")}-{index + 1}
            </button>
          </h2>
          <div
            id={`panelsStayOpen-collapseEdu${train?.id}`}
          ...
          >
            <div
              ...
              style={{
                position: "relative",
                opacity: train?.import ? ".5" : "1",
                pointerEvents: train?.import ? "none" : "",
              }}
            >
              <Row
                className=""
                style={{ pointerEvents: train.edit ? "" : "none" }}
              >
                {train?.import && (
                  <div
                ...
                <Col md={6} xs={12}  className=" mb-2">
                  <label htmlFor="" className="form-label">
                    {" "}
                    {contentLabel("TrainingName", "nf Training Name")}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setResumeTrain((prevTrainings) => {
                        return prevTrainings.map((item) =>
                          item.id === train.id
                            ? { ...item, trainingName: e.target.value }
                            : item
                        );
                      });
                    }}
                  />
                  <InputLimit value={train?.trainingName} />
                </Col>
                ...
                <Col  md={6} xs={12} className=" mb-4">
                  ...
                  <CreatableSelect
                    ...
                    // options={accordionStates[train.id]?.educationSuggestionsData || []}
                    options={(() => {
                      const options =
                        accordionStates[train.id]?.educationSuggestionsData ||
                        [];
                    ...
                    isLoading={accordionStates[train.id]?.isApiLoader}
                      menuIsOpen={accordionStates[train.id]?.isApiLoader ? false : undefined}
                    inputValue={
                      accordionStates[train.id]?.institutionFirstWord || ""
                    }
                    value={
                      accordionStates[train.id]?.institution
                        ? {
                            label: accordionStates[train.id]?.institution,
                            value: accordionStates[train.id]?.institution,
                          }
                        : null
                    }
                    // menuIsOpen={true}
          ...
          <span style={{ color: "var(--primary-color)" }}>{contentLabel('Create', 'nf Create')}</span>
                      ...
                      <label className="text-label form-label d-flex align-items-center white-space-no-wrap">
                        {(
                          content[selectedLanguage]?.find(
                            (item) => item.elementLabel === "TrainingStartedOn"
                          ) || {}
                        ).mvalue || "nf When did you start your Training "}{" "}
                        <span className="text-danger ms-1">*</span>
                      </label>
                        ...
                        <DatePicker
                          ...
                          selected={
                            train?.startYear
                              ? new Date(
                                  parseInt(
                                    new Date(train?.startYear, 0, 1).getTime()
                                  )
                                )
                              : null
                          }
                  ...
                  <Col xs={6} className="ps-1 ps-md-3">
                    <div className="mb-4 d-flex flex-column justify-content-between">
                      ...
                      <MonthDropdown
                        ...
                        selectedMonthNumber={train.startMonth}
                        selectedYear={new Date(train?.startYear, 0, 1)}
                      />
                    </div>
                  </Col>
                </Col>
                  ...
                  <div className="mb-4">
                    <label className="d-flex justify-content-between form-label text-label">
                      ...
                      <div className="d-flex align-items-center justify-content-center">
                        <input
                          ...
                          type="checkbox"
                          name="online"
                          checked={train?.location?.includes("Online")}
                          onChange={(e) => {
                            setOnline(!online);
                          }}
                        />
                    ...
                    <MultiSelect
                      setLocationData={(newLocation) => {
                        setResumeTrain((prevTrain) => {
                          return prevTrain.map((item) =>
                            item.id === train.id
                ...
                <Col class=" col-12 mb-3">
                  <BriefDescriptionTextArea
                    label={contentLabel(
                      "BriefDescTraining",
                      "nf Brief decription about training"
                    )}
                    htmlFor={"TrainingBriefDescription"}
                    isRequired={false}
                    textareaClass={""}
