...
const ResumeEdu = ({ edu, setResumeEdu, index, updateResumeDetails }) => {
  ...
  }, [accordionStates[edu.id]?.institutionFirstWord, edu.id]);

  useEffect(() => {
    const currentAccordion = accordionStates[edu.id];

    if (currentAccordion?.courseFirstWord?.trim()?.length > 0) {
      const controller = new AbortController();
      const signal = controller.signal;

      updateAccordionState(edu.id, "isCourseApiLoader", true);

      debouncedApiRequest(
        degreeSearchSuggestions,
        currentAccordion.courseFirstWord,
        selectedLanguage,
        (data) => {
          const formattedData = data?.map((data) => ({
            value: data?.combined,
          ...
          updateAccordionState(
            edu.id,
            "courseSuggestionsData",
            formattedData || []
          );
          updateAccordionState(edu.id, "isCourseApiLoader", false);
          updateAccordionState(edu.id, "isApiLoader", false);
        },
        () => updateAccordionState(edu.id, "isCourseApiLoader", false),
        signal
      ...
      updateAccordionState(edu.id, "courseSuggestionsData", []);
    }
  }, [accordionStates[edu.id]?.courseFirstWord, edu.id]);
  ...
  const handleCourseChange = (selectedOption) => {
    ...
    const isExisting = accordionStates[edu.id]?.courseApiData?.some(
      (inst) => inst.label === selectedOption?.label
    );
      ...
      <div class="accordion w-100 resumeAccordion" id="accordionPanelsStayOpenExample">
        <div class="accordion-item  ">
          <h2 class="accordion-header ">
            ...
            >
              {/* {edu?.organization} */}
              {contentLabel("Education", "nf Education")}-{index + 1}
            </button>
          </h2>
          <div
              ...
              >
                ...
                <Col lg={6} className="mb-4">
                  ...
                  <CreatableSelect
                    ...
                    inputValue={accordionStates[edu.id]?.courseFirstWord || ""}
                      ...
                      if (event.action === "menu-close" && false) {
                        if (!value) {
                          updateAccordionState(
                            edu.id,
                            "courseFirstWord",
                            accordionStates[edu.id]?.courseFirstWord
                          );
                        }
                      } else
                        updateAccordionState(edu.id, "courseFirstWord", value);
                    }}
                    placeholder={contentLabel(
                      "SelectYourCourse",
                      "nf Select Your Course"
                    )}
                    onChange={handleCourseChange}
                    options={(() => {
                      const options =
                        accordionStates[edu.id]?.courseSuggestionsData || [];
                      // console.log(options)
                    ...
                    isLoading={accordionStates[edu.id]?.isCourseApiLoader}
                    value={
                      accordionStates[edu.id]?.courseName
                        ? {
                            label: accordionStates[edu.id]?.courseName,
                            value: accordionStates[edu.id]?.courseName,
                          }
                        : null
                    }
          ...
          <span style={{ color: "var(--primary-color)" }}>{contentLabel('Create', 'nf Create')}</span>
                  ...
                  <Col xs={6} className="pe-1 pe-md-3">
                    <div className=" d-flex flex-column justify-content-between">
                      <label className="text-label form-label d-flex align-items-center white-space-no-wrap">
                        {contentLabel(
                          "EducationStartedOn",
                          "nf When did you started your education?"
                        )}
                        <span className="text-danger ms-1">*</span>
                      </label>
                ...
                <Col xs={12} class="mb-3">
                  <BriefDescriptionTextArea
                    label={contentLabel(
                      "BriefDescEdu",
                      "nf Brief decription about education"
                    )}
                    htmlFor={"EducationBriefDescription"}
                    textareaClass={""}
                    id={"EducationBriefDescription"}
