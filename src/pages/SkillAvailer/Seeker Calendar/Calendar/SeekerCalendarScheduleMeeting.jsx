...
import "../../../SkillingAgencies/components/candidate management/CalendarScheduleMeeting.css"
import { emptyCalendarJdData } from '../../../../reducer/SkillSeeker/JdData/JdDataSlice';
...
const SeekerCalendarScheduleMeeting = ({row }) => {
 
 const {calendarJdData} = useSelector(state => state.JdDataSlice);
 ...
 const contentLabel = useContentLabel();
  ...
  const dummySkillOwners=[{id: '1', name: calendarJdData?.firstName, email: calendarJdData?.email}]
 ...
 const dispatch = useDispatch();
  ...
  const [ownnerMail, setOwnnerMail] = useState([calendarJdData?.email]);
...
if(isValue){
 setAgencyTimezone(matchedZones?.[0])
  ...
  console.log("seekeremail",globalAttendees);
  console.log("owneremail",globalExtraAttendee);
...
useEffect(() => {
  const fetchData = async () => {
    ...
    await AccountDetails(calendarJdData?.account?.accountId,false);   
    setIsLoading(false)
  ...
  fetchData();
  console.log("calendarJdData",calendarJdData);
  console.log("calendarJdData",calendarJdData?.account?.accountId);
  
}, [calendarJdData, userDetails]);
...
function formatWithTimezone(input, tz) {
  ...
  const handleMeetingSubmit = async (e) => {
    ...
    if (extraAttendee.trim()) {
      emails.add(extraAttendee.trim());
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        closeGlobalModal();
        navigate(
          `/skill-seeker/Opportunities/Candidate-management?Title=${queryParams.get("Title")}&Id=${queryParams.get("Id")}&Type=Opportunity`
        );
        setTimeout(() => {
          fetchEvents();
        }, 1000);
  ...
  return (
    ...
    {isLoading ? 
      ...
      <CalendarComponent
        ...
        skillowner={calendarJdData?.firstName}
        isDashboard={false}
        adminAbbr={adminabbr}
        userAbbr={userabbr}
...
export default React.memo(SeekerCalendarScheduleMeeting);